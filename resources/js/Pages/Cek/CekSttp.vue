        ...
        <div class="col">
             <Toolbar class="p-mb-4">
                <template #left>
                     ...
                     <span class="ml-2">
                        Tanggal : <InputText type="date" v-model="lazyParams.start"/> s/d <InputText type="date" v-model="lazyParams.end"/>
                        <Button @click="cari" class="p-button-raised p-button-success ml-2" label="Proses" />
                     </span>
                </template>
            </Toolbar>
            ...
            <Column field="jml_masuk" header="Jumlah Masuk">
                 <template #body="slotProps">
                     {{ slotProps.data.jml_masuk ? slotProps.data.jml_masuk : 0  }}
                </template>
            </Column>
...
import { getCekSttp } from '../../Api/cek.api.js';
...
export default {
   ...
   data() {
    ...
    methods:{
        async loadLazyData() {
            this.loading = true;
            const res = await getCekSttp({page:this.lazyParams.page + 1, search:this.lazyParams.search, start:this.lazyParams.start,end:this.lazyParams.end});
